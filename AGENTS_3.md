...
> gap-fill against real code, not a from-scratch build. Status per
> "Required Subsystem" section below:
>
> | # | Subsystem | Status | Where |
> |---|---|---|---|
> | 1 | Visualization design system (`qortex.viz`) | Not built as a new package — `_colors.py`/`_html.py`/`showcase.py` already cover theme/typography/layout; a new 13-module package would duplicate them. | `src/qortex/visualize/_colors.py`, `_html.py` |
> | 2 | Dataset readiness figures | Not done — `eda/quality.py` computes the data, no card-style figure renders it yet. | — |
> | 3 | Participant metadata figures | **Done.** Parser + dirty-categorical-value detector + violin/box/strip figure with metric cards and an invalid-value banner. Caught and fixed a real bug (a raw `"M,"` value was silently merged into the `"M"` group before the fix). | `src/qortex/eda/participants.py` |
> | 4 | Neuroimaging preview, quality-scored slice selection | **Done.** `_best_axial_index()` scores a bounded set of candidate slices by brain-tissue coverage instead of the geometric midpoint; wired into `VolumeViewer.ortho()` and the dataset audit thumbnail. Verified on real data: midpoint z=96 vs. selected z=111. | `src/qortex/visualize/volume.py`, `_audit.py` |
> | 5 | NeuroAI detection/segmentation overlays | **Done.** Segmentation showcase was already mature. Added `render_detection_showcase()` — annotated image + confidence table + per-class legend + threshold/NMS caption — mirroring the segmentation board layout (previously only a bare per-frame box drawer existed). | `src/qortex/neuroai/showcase.py` |
> | 6 | Model registry, multi-backend | **Done for torchvision + keras.** Added real adapters (tested end-to-end with actual inference and real parameter counts, no weight downloads without explicit `pretrained=True`), wired into the existing adapter factory, added curated registry entries, and added `list_models()` / `get_model_card()` / `register_model()` convenience API on top of the existing curated `ModelContractEntry` registry (no parallel catalog). braindecode/huggingface/monai/onnx/ultralytics adapters already existed. | `src/qortex/neuroai/models/torchvision_adapter.py`, `keras_adapter.py`, `_registry.py`, `_contracts.py` |
> | 7 | Dataset explorer / search dashboard | Not done — broad UI-composition work (filter chips, sortable table, file tree, export workflow) with no single narrow entry point; only a bare coverage heatmap exists today. | — |
> | 8 | Viewer / QC dashboard, FD/DVARS | **Done.** DVARS is now computed directly from consecutive BOLD frames (no confounds file required); FD is computed from real motion parameters (Power et al. formula) when present, falling back to "unavailable" rather than fabricating motion. Verified against a synthetic run with injected motion spikes — both traces spike at the correct frames. | `src/qortex/visualize/volume.py` (`fmri_summary`, `_LazyNIfTI.dvars`) |
> | 9 | Connectivity / signal / reproducibility figures | **Done for connectivity.** Heatmap + thresholded circular network graph + hub-degree/summary panel rendering the existing, previously-unplotted `neuroclassic.connectivity` output. PSD/spectrogram/reproducibility-DAG/provenance-table figures not attempted. | `src/qortex/visualize/connectivity.py` |
>
> **Not implemented, with reason:**
> - Full `qortex.viz` design-system package and full `qortex.models` package as new top-level packages — the working code they'd re-house already exists in place; rebuilding it under new names was judged busywork over benefit.
> - Dataset explorer dashboard, PSD/spectrogram figures, reproducibility DAG/provenance-hash table — each is a genuinely broad, multi-file UI composition, not a narrow gap; none were started.
> - Test coverage for `visualize/` (~7,500 lines, still untested) — explicitly deferred by request ("code only for now").
>
> **Verification performed:** `python -m compileall`, full `pytest` (197 passed; 3 pre-existing failures in `test_neuroclassic_advanced.py` from an unrelated `numpy.trapezoid` version issue, confirmed untouched by this work), and real (non-mocked) execution of every new code path — including actual torchvision/keras inference, real FD/DVARS detection of injected synthetic artifacts, and visual inspection of all four generated gallery figures in `artifacts/qortex_gallery/`.
